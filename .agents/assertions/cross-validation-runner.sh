...
if len(args) % 2 != 0:
    print("  ⚠ ERROR: Invalid number of arguments. Expected pairs of --report-name path.")
    print(f"  Provided: {len(args)} arguments (expected even number)")
    sys.exit(1)
...
# Extract the final result from Python output
...
if all_passed:
    print(f"  ✅ Cross-validation PASSED - pipeline can proceed to pipe/obs")
    print(f"SUCCESS: all_passed=true")
else:
    print(f"  ❌ Cross-validation FAILED - pipeline blocked")
    print(f"  Recommendations:")
    for result in validation_results:
        if not result.get("passed", False):
            print(f"    - Fix {result['rule_id']}: {result.get('reason', 'Validation failed')}")
    print(f"FAILURE: all_passed=false")

print(f"FINAL_RESULT: {'PASS' if all_passed else 'FAIL'}")
PYTHON_SCRIPT

# Extract the final result
FINAL_RESULT=$(echo "$FINAL_RESULT" | grep "FINAL_RESULT:" | cut -d: -f2 | tr -d ' ')

echo "Final validation result: $FINAL_RESULT"
