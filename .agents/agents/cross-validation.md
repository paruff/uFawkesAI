...
You run after `test-execution`, `review`, `build-review`, and `security` have all completed, and you validate pairwise consistency between their outputs and their sources (`spec`, `design`, `build`, `test`). If any validation fails, you block progression to `pipe` and `obs`.
...
3. **Run the cross-validation runner** (`.agents/assertions/cross-validation-runner.sh`) to:
   - Read the relevant agent reports (spec, design, build, test, test-execution, review, build-review, security)
...
The cross-validation process validates these 6 pairwise relationships:
...
- Never skip cross-validation — it is the final gate before pipe/obs
- Never accept a partial validation — all 6 rules must pass
